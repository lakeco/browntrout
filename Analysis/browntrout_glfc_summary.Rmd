...
df <- read.csv("~/projects/browntrout/Data/Raw/bt_glfc.csv") # previously downloaded - brown trout only
...
stglkup <- read_xlsx("~/projects/browntrout/Data/Lookup/GLFSD_NewDataSubmission_Template.xlsx", sheet = "Lifestage")
...
# stocking location centroids from GLFC 10min grid layer (exact coordinates not available for lots of events)

# check how many sites are missing exact coordinates

missing_coords <- df %>%
  select("latitude", "longitude") %>%
  summarise_all(funs(sum(is.na(.))))

df10 <- read.csv("~/projects/browntrout/Data/Spatial/gl_10min_grid.csv", stringsAsFactors = FALSE)

df10 <- df10%>% rename(
  grid_10min = CELL10MIN,
  lat_grid = ycoord,
  lon_grid = xcoord
)


df <- left_join(df, df10, by = c("grid_10min","lake"))
...
map <- leaflet(width = "100%", height = 800) %>%
  ...
  addCircleMarkers(data = sd, lat = ~lat_grid, lng = ~lon_grid, radius = ~number/1000,
                   stroke = TRUE,
                   color = "black",
                   weight = 1,
                   ...
                   ("<b><font size=+1><font color=#5499C7>",stocked$location_primary, "</b></font size></font color><br>",
                     ...
                     "<br><b>LAT: </b>", stocked$lat_grid,
                     "<br><b>LON: </b>", stocked$lon_grid
                   ))%>%
